...
public class MiaoshaService {
    ...
    @Transactional
    public OrderInfo miaosha(MiaoshaUser user, ProductMiaoShaVo goods) {
        //减库存 下订单 写入秒杀订单
        boolean success = productService.reduceStock(goods);
        if (success) {
...
/*
    public String updateUrl(RandomProductUrlVO randomProductUrlVO) {
        ...
        if (length < 20 || length > 200)
            throw new RuntimeException(ResultStatus.DATA_ILLEGAL_EXCEPTION.getMessage());
        // 判断产品id是否为空
        Long productId = randomProductUrlVO.getProductId();
        if (productId == null) {
            throw new RuntimeException(ResultStatus.DATA_NOT_NULL_EXCEPTION.getMessage());
        }

        // 随机算法生成链接
        String url = RandomUtils.createRandomUrl(length);
