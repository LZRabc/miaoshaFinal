...
public class LoginController {
    ...
    @PostMapping("/login")
    @ResponseBody
    public ResultGeekQ<loginPojo> dologinL(HttpServletResponse response, @RequestBody LoginVo loginVo) {
    ...
    @ResponseBody
    public ResultGeekQ<loginPojo> autoLogin(@RequestParam("token")String token) {
        ResultGeekQ<loginPojo> result = ResultGeekQ.build();
        ...
        logger.info("auto login:"+user.getRealName());
        loginPojo data=new loginPojo();
        data.setRealName(user.getRealName());
        data.setUsername(user.getNickname());
        result.setData(data);
